...
function Project() {
    ...
    useEffect(() => {
        setFiltered(
            activeTab === 'all'
                ? projects
                : projects.filter((p) => p.classTh === activeTab)
        )
    }, [activeTab, projects])
    ...
    useEffect(() => {
        try {
            ...
            setProjects(data)
            const classThs = [...new Set(data.map((p) => p.classTh))]
                .sort()
                .reverse()
            if (!classThs.includes('14')) 
                classThs.unshift('14')
            setTabs(classThs)
        } catch (e) {
    ...
    const handleTabClick = (tab) => {
        ...
        setFiltered(
            tab === 'all'
                ? projects
                : projects.filter((p) => p.classTh === tab)
        )
    }
    ...
    return (
        <div className="min-h-screen mx-auto bg-black">
            <div className="w-4/5 mx-auto py-30 md:py-40">
                <div
                    className="pb-8 mx-auto text-center font-extrabold md:w-fit md:pb-12 md:pr-20 md:border-b-2 md:text-start md:mx-0">
                ...
                <ProjectTabs activeTab={activeTab} onTabClick={handleTabClick} tabs={tabs}/> {
                    filtered.length > 0
                        ? (
                            <div
                                className="grid w-full grid-cols-2 gap-5 mx-auto mt-8 text-white md:gap-8 sm:grid-cols-3 sm:w-full xl:gap-12 lg:grid-cols-4">
                                {
                                    filtered.map((p) => (
                                        <div
                                            key={p.id}
                                            ...
                                            <div className="relative">
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        className="px-3 py-1.5 text-sm text-white bg-blue-500 rounded-md font-bold sm:px-3 sm:py-2 sm:text-sm md:px-4 md:py-2 md:text-base lg:text-base xl:px-5 xl:py-2 xl:text-lg">
                                                        사이트 보러가기
                                                    </span>
                                                </div>
                                            </div>
                                            <div className="p-2">
                                                <div
                                                    className="my-1 text-sm xl:my-2 font-bold min-[500px]:text-lg lg:text-xl xl:text-[23px]">{p.title}</div>
                        ...
                        : (
                            <div className="flex flex-col items-center justify-center w-full mt-24 mb-32">
                                <div
                                    className="text-[20px] sm:text-[24px] text-[#A2A2A2] fontBold mb-4 text-center break-keep">
                                    아직 등록된 프로젝트가 없습니다.
                                </div>
                                <div
                                    className="text-[16px] sm:text-[18px] text-[#666666] fontRegular text-center break-keep">
